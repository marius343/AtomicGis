...
void drawtext_left(const t_point& text_center, const std::string& text, const t_bound_box& bounds, float tolerance);
void drawtext_left(float xc, float yc, const std::string& str_text, float boundx, float boundy);

//!!! CUSTOM FUNCTION !!!
// checks if the text box will be drawn
bool willDrawText(float xc, float yc, const std::string& text, float boundx = FLT_MAX, float boundy = FLT_MAX, labels * sLabel = NULL);

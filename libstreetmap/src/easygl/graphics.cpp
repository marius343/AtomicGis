 ...
 */
int xworld_to_scrn(float worldx) {
 ...
 */
int yworld_to_scrn(float worldy) {
...
rect_off_screen(float x1, float y1, float x2, float y2) {
    ...
    //offset
    if (x1 > xmax + dx * 0.1 && x2 > xmax + dx * 0.1)
        return (1);

    //offset
    if (y1 < ymin - dy * 0.03 && y2 < ymin - dy * 0.03)
        return (1);
...
void drawtext_in_left(const t_bound_box& bbox, const std::string& text, float tolerance) {
    drawtext_left(bbox.get_center(), text, bbox, tolerance);
}
...
void drawtext_left(
    ...
    if (tolerance_bounds.intersects(text_center) != true) {
        cout << "The center of the text \"" << text << "\" isn't in its bounding box.\n";
        //        printf("the center of the text \"%s\" isn't in its bounding box", text.c_str());
    }
    t_point bottomleft_bounds = text_center - tolerance_bounds.bottom_left();
    t_point topright_bounds = tolerance_bounds.top_right() - text_center;
    t_point min_bounds(
            min(bottomleft_bounds.x, topright_bounds.x),
            min(bottomleft_bounds.y, topright_bounds.y)
            );
    min_bounds *= 2;
    drawtext_left(text_center.x - bounds.get_width() / 2 + 10, text_center.y, text, min_bounds.x, min_bounds.y);
}

void drawtext_left(float xc, float yc, const std::string& str_text, float boundx, float boundy) {
    // Need a C-string to call the low-level (X11 or win32) apis.
    const char* text = str_text.c_str();
    int text_byte_length = strlen(text);
    float angle = PI * gl_state.currentfontrotation / 180.;
...
bool willDrawText(float xc, float yc, const std::string& str_text, float boundx, float boundy, labels * sLabel) {
    // Need a C-string to call the low-level (X11 or win32) apis.
    const char* text = str_text.c_str();
    int text_byte_length = strlen(text);
    float angle = PI * gl_state.currentfontrotation / 180.;
    ...
    if (
        ...
        return false; // Text bigger than user wants; don't draw
    }
    ...
    if ((fabs(coord_width) > fabs(boundx)) || (fabs(coord_height) > fabs(boundy))) {
        return false; // don't draw if it won't fit in xbound or ybound.
    }
